...
KP_FESTIVALS = [
    ...
    ("Chaitra",    "Shukla",   8, "Chaitra Ashtami",           "#9C27B0"),
    ("Chaitra",    "Shukla",   9, "Ram Navami",                "#FF9800"),
    ("Chaitra",    "Shukla",  15, "Chaitra Purnima",           "#B8862A"),
    # ── VAISHAKHA (Apr-May) ───────────────────────────────────────────────────
    ("Vaishakha",  "Shukla",   3, "Akshaya Tritiya",           "#FFD700"),
    ("Vaishakha",  "Shukla",   8, "Vaishakha Ashtami (Sharika)","#9C27B0"),
    ("Vaishakha",  "Shukla",  15, "Vaishakha Purnima / Buddha Jayanti","#B8862A"),
    # ── JYESHTHA (May-Jun) ────────────────────────────────────────────────────
    ...
    ("Jyeshtha",   "Shukla",  15, "Jyeshtha Purnima",          "#B8862A"),
    # ── ASHADHA (Jun-Jul) ─────────────────────────────────────────────────────
    ("Ashadha",    "Shukla",   2, "Ratha Yatra",               "#FF7043"),
    ("Ashadha",    "Shukla",  15, "Guru Purnima",              "#B8862A"),
    # ── SHRAVANA (Jul-Aug) ────────────────────────────────────────────────────
    ("Shravana",   "Shukla",   5, "Nag Panchami",              "#2A7A6A"),
    ("Shravana",   "Shukla",  15, "Raksha Bandhan",            "#FF7043"),
    ...
    ("Bhadrapada", "Krishna",  8, "Jee Kounth Satam / Thar Satam","#8B1C30"),
    ("Bhadrapada", "Krishna", 30, "Mahalaya Amavasya",         "#555"),
    # ── ASHWINA (Sep-Oct) ─────────────────────────────────────────────────────
    ("Ashwina",    "Shukla",   1, "Navratri Begins",           "#D4722A"),
    ...
    ("Ashwina",    "Shukla",  15, "Sharad Purnima / Kojagiri", "#B8862A"),
    # ── KARTIKA (Oct-Nov) ─────────────────────────────────────────────────────
    ("Kartika",    "Krishna", 13, "Dhanteras",                 "#FFD700"),
    ("Kartika",    "Krishna", 14, "Choti Diwali / Naraka Chaturdashi","#FFD700"),
    ("Kartika",    "Krishna", 30, "Diwali / Deepavali",        "#FFD700"),
    ("Kartika",    "Shukla",   1, "Govardhan Puja",            "#FF9800"),
    ("Kartika",    "Shukla",   2, "Bhai Dooj",                 "#FF7043"),
    ("Kartika",    "Shukla",  11, "Chourams (Dev Uthani Ekadashi)","#7C4DFF"),
    ("Kartika",    "Shukla",  12, "Tulsi Vivah",               "#4CAF50"),
    ("Kartika",    "Shukla",  15, "Kartika Purnima",           "#B8862A"),
    ...
    ("Margashirsha","Shukla", 15, "Margashirsha Purnima",      "#B8862A"),
    # ── PAUSHA (Dec-Jan) ──────────────────────────────────────────────────────
    ("Pausha",     "Krishna",  8, "Tila Ashtami (Til Oil Lamp)","#FF9800"),
    ("Pausha",     "Shukla",  15, "Pausha Purnima",            "#B8862A"),
    # ── MAGHA (Jan-Feb) ───────────────────────────────────────────────────────
    ("Magha",      "Shukla",   5, "Vasant Panchami / Saraswati Puja","#FFD700"),
    ("Magha",      "Shukla",  15, "Magha Purnima",             "#B8862A"),
...
with tabs[0]:
    ...
    with home_r:
        ...
        try:
            ...
            """, unsafe_allow_html=True)
            # Today's festivals
            _today_fests = _FEST_LOOKUP.get(
                (home_p['masa'], home_p['paksha'], home_p['tithi_num']), []
            )
            if _today_fests:
                _fest_html = "".join(
                    f'<span style="display:inline-block;background:{fc};'
                    f'color:white;font-size:9px;padding:2px 8px;border-radius:10px;'
                    f'margin:2px;font-family:Cinzel,serif;letter-spacing:0.5px">{fn}</span>'
                    for fn, fc in _today_fests
                )
                st.markdown(
                    f'<div style="text-align:center;margin-top:6px">{_fest_html}</div>',
                    unsafe_allow_html=True
                )
        except Exception:
            pass
...
with tabs[1]:
    ...
    # ── Session state init — handle all date overrides BEFORE widget renders ──
    if "pan_cal_click" in st.session_state:
        st.session_state["pan_date"] = st.session_state.pop("pan_cal_click")
    if "pan_today_click" in st.session_state:
        st.session_state.pop("pan_today_click")
        st.session_state["pan_date"] = today
        st.session_state["cal_view_year"]  = today.year
        st.session_state["cal_view_month"] = today.month
    if "pan_date" not in st.session_state:
        st.session_state["pan_date"] = today

    # ── TOP ROW: State | District | Date (inline) ─────────────────
    ctrl1, ctrl2, ctrl3, ctrl4 = st.columns([1.6, 1.6, 1.2, 0.8])
    with ctrl1:
        _pan_state = st.selectbox("State · राज्य", _STATES_LIST,
                                   index=_STATE_IDX.get("Jammu & Kashmir", 0),
                                   key="pan_state")
    with ctrl2:
        _pan_dists = _DIST_LISTS.get(_pan_state, [])
        _pan_dist  = st.selectbox("District · जिला", _pan_dists,
                                   index=_DIST_IDX.get(_pan_state, {}).get("Jammu", 0),
                                   key="pan_dist")
    with ctrl3:
        p_date = st.date_input("Date · तारीख", key="pan_date")
    with ctrl4:
        if st.button("Today", key="pan_today", use_container_width=True):
            st.session_state["pan_today_click"] = True
            st.rerun()

    _pan_coords = INDIA_STATES[_pan_state][_pan_dist]
    p_lat, p_lon = _pan_coords[0], _pan_coords[1]
    p_loc = f"{_pan_dist}, {_pan_state}"

    # ── MAIN ROW: Masa info box | Calendar ────────────────────────
    centre_col, right_col = st.columns([1.8, 4.5])
    ...
    try:
        ...
        with right_col:
            ...
            # Keep calendar view independent of selected date
            if "cal_view_year"  not in st.session_state:
                st.session_state["cal_view_year"]  = p_date.year
            if "cal_view_month" not in st.session_state:
                st.session_state["cal_view_month"] = p_date.month

            nav_prev, nav_month_col, nav_year_col, nav_next = st.columns([0.5, 1.5, 1.2, 0.5])

            with nav_prev:
                if st.button("‹", key="cal_prev", help="Previous month",
                             use_container_width=True):
                    m = st.session_state["cal_view_month"] - 1
                    y = st.session_state["cal_view_year"]
                    if m < 1:
                        m = 12; y -= 1
                    st.session_state["cal_view_month"] = m
                    st.session_state["cal_view_year"]  = y
                    st.rerun()
            ...
            with nav_month_col:
                ...
                def _on_month_change():
                    st.session_state["cal_view_month"] = MONTH_NAMES.index(st.session_state["cal_sel_month"]) + 1
                st.selectbox(
                    "Month", MONTH_NAMES,
                    index=st.session_state["cal_view_month"] - 1,
                    key="cal_sel_month", label_visibility="collapsed",
                    on_change=_on_month_change,
                )

            with nav_year_col:
                def _on_year_change():
                    st.session_state["cal_view_year"] = st.session_state["cal_sel_year"]
                st.selectbox(
                    "Year", YEAR_RANGE,
                    index=_YEAR_IDX.get(st.session_state["cal_view_year"],
                                        _YEAR_IDX.get(today.year, 0)),
                    key="cal_sel_year", label_visibility="collapsed",
            ...
            month_name = f"{MONTH_NAMES[cal_month - 1]} {cal_year}"
            ...
            # ── Month title + weekday header row ──────────────────────
            st.markdown(
                f'<div style="background:white;border:1px solid var(--border);'
                f'border-radius:10px 10px 0 0;padding:5px 8px 0;">'
                f'<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:2px;'
                f'color:var(--walnut-mid);text-align:center;padding-bottom:4px;'
                f'border-bottom:1px solid var(--border)">{month_name.upper()}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
            hdr_cols = st.columns(7)
            for wi, wd_name in enumerate(WD):
                hdr_cols[wi].markdown(
                    f'<div style="font-family:Cinzel,serif;font-size:8px;color:var(--muted);'
                    f'text-align:center;padding:2px 0;background:white;'
                    f'border-left:{"1px solid rgba(184,134,42,.1)" if wi > 0 else "none"}">'
                    f'{wd_name[:2]}</div>',
                    unsafe_allow_html=True
                )

            # ── Day rows ──────────────────────────────────────────────
            all_days = [None] * first_wd + list(range(1, last_day.day + 1))
            rem = (7 - len(all_days) % 7) % 7
            all_days += [None] * rem

            for row_start in range(0, len(all_days), 7):
                week    = all_days[row_start:row_start + 7]
                day_cols = st.columns(7)
                for col_i, dn in enumerate(week):
                    with day_cols[col_i]:
                        if dn is None:
                            st.markdown(
                                '<div style="min-height:32px;background:white;'
                                'border:1px solid rgba(184,134,42,.05)"></div>',
                                unsafe_allow_html=True
                            )
                            continue
                        ...
                        # Tithi label and colour (short form)
                        if tnum == 30:
                            ts = "Am"; tc = AM_CLR
                        elif tnum == 15:
                            ts = "Pu"; tc = PU_CLR
                        else:
                            ts = f"{'S' if paksha == 'Shukla' else 'K'}{tnum}"
                            tc = "var(--saffron)"

                        # Cell background
                        if is_sel:
                            bg  = SEL_CLR; nc = "#DEB85A"
                            brd = "2px solid #DEB85A"
                            tc  = "rgba(222,184,90,.8)"
                        elif is_td:
                            bg  = "rgba(184,134,42,.12)"; nc = "var(--walnut)"
                            brd = "1px solid rgba(184,134,42,.35)"
                        elif not ausp:
                            bg  = "rgba(192,48,48,.05)"; nc = "var(--ink)"
                            brd = "1px solid rgba(184,134,42,.08)"
                        else:
                            bg  = "white"; nc = "var(--ink)"
                            brd = "1px solid rgba(184,134,42,.1)"

                        # Festival dots (max 2)
                        dots = "".join(
                            f'<span style="display:inline-block;width:4px;height:4px;'
                            f'border-radius:50%;background:{fc};margin:0 0.5px"></span>'
                            for _, fc in fests[:2]
                        )
                        fest_line = ""
                        if fests:
                            fc0 = "#DEB85A" if is_sel else fests[0][1]
                            fname_short = " ".join(fests[0][0].split()[:3])
                            fest_line = (
                                f'<div style="font-size:9px;font-weight:600;color:{fc0};'
                                f'line-height:1.2;margin-top:2px;overflow:hidden;'
                                f'text-overflow:ellipsis;white-space:nowrap;'
                                f'padding:0 2px">'
                                f'{fname_short}</div>'
                            )

                        # Render styled HTML cell (display only)
                        st.markdown(
                            f'<div style="background:{bg};border:{brd};'
                            f'border-radius:6px;min-height:44px;'
                            f'padding:3px 2px 3px;text-align:center">'
                            f'<div style="font-family:Cinzel,serif;font-size:11px;'
                            f'font-weight:500;color:{nc};line-height:1">{dn}</div>'
                            f'<div style="font-size:7px;color:{tc};margin-top:1px;'
                            f'line-height:1.1">{ts}</div>'
                            f'<div style="margin-top:1px">{dots}</div>'
                            f'{fest_line}'
                            f'</div>',
                            unsafe_allow_html=True
                        )
            ...
            # Festival legend — colour key
            legend_items = [
                ("S# Shukla · K# Krishna", "var(--muted)"),
                ("Purnima", PU_CLR),
                ("Amavasya", AM_CLR),
                ("Herath / Shivratri", "#e53935"),
                ("Navreh / Navratri", "#4CAF50"),
                ("Zyeth Ashtami", "#DEB85A"),
                ("Janmashtami", "#3F51B5"),
                ("Diwali / Ekadashi", "#FFD700"),
                ("Nag Panchami", "#2A7A6A"),
                ("Pitru Paksha", "#8B1C30"),
                ("Chourams / Tulsi Vivah", "#7C4DFF"),
                ("Sonth / Tila Ashtami", "#26C6DA"),
            ]
            legend_dots = "".join(
