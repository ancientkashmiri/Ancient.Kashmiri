...
# Set ephemeris path and fix Lahiri ayanamsa at module level
swe.set_ephe_path('')
swe.set_sid_mode(swe.SIDM_LAHIRI)   # set once globally — ensures all calc_ut calls use Lahiri
tf = TimezoneFinder()
...
def _ayanamsa_cached(jd: float) -> float:
    ...
    return swe.get_ayanamsa_ut(jd)   # _ut variant for UT Julian days — more precise
...
_CALC_FLAGS = swe.FLG_SWIEPH | swe.FLG_SPEED   # explicit flags for maximum precision
...
def get_planet_lon(jd: float, planet: int) -> float:
    ...
    Uses swe.FLG_SWIEPH | swe.FLG_SPEED for maximum precision.
    Uses get_ayanamsa_ut (UT Julian Day) — correct for all our time inputs.
    planet == -1 means Ketu (= Rahu + 180°).
    """
    ayanamsa = _ayanamsa_cached(jd)
    if planet == -1:
        rahu_trop = swe.calc_ut(jd, swe.TRUE_NODE, _CALC_FLAGS)[0][0]
        return (rahu_trop - ayanamsa + 180.0) % 360.0
    trop_lon = swe.calc_ut(jd, planet, _CALC_FLAGS)[0][0]
...
def get_sidereal_asc(jd: float, lat: float, lon: float) -> float:
    ...
    ayanamsa = _ayanamsa_cached(jd)
    houses   = swe.houses_ex(jd, lat, lon, b'P', swe.FLG_SWIEPH)  # explicit flags
...
_NAK_AS      = int(round(_NAK_SPAN * 3600.0))   # 48000 arc-seconds — precomputed
_PADA_AS     = _NAK_AS // 4                      # 12000 arc-seconds — precomputed
...
def lon_to_nakshatra(lon: float) -> Tuple[str, int, float]:
    ...
    """
    lon = lon % 360.0
    lon_as     = int(round(lon * 3600.0))
    nak_idx    = min(lon_as // _NAK_AS, 26)
    rem_as     = lon_as - nak_idx * _NAK_AS
    pada       = min(rem_as // _PADA_AS + 1, 4)
...
def get_masa(jd: float, sun_lon: float = None) -> str:  # sun_lon kept for API compat
    """Amāvasyānta lunar month name.

    Traditional rule (Drik Panchang / Surya Siddhanta):
      - A regular month is named after the Sun's rashi at its ending Amavasya.
      - An Adhika (intercalary) month takes the name of the NEXT regular month
        that follows it, because the Adhika month precedes and "belongs to" that
        regular month.

    So if Sun is in Vrishabha at both the current and next Amavasya:
      - The current lunation is Adhika → named Adhika Jyeshtha (the month it precedes)
      - The next regular lunation (Sun in Mithuna) is Nija Jyeshtha
    """
    end_am   = _current_am_start(jd) if get_tithi(jd)[0] == 30 else _find_next_amavasya(jd)
    start_am = _find_prev_amavasya(jd)

    end_rashi   = int(get_planet_lon(end_am,   swe.SUN) / 30.0) % 12
    start_rashi = int(get_planet_lon(start_am, swe.SUN) / 30.0) % 12

    if start_rashi == end_rashi:
        # Adhika masa — name it after the NEXT regular month (one Amavasya further)
        next_am = _find_next_amavasya(end_am + 1.0)
        return MASA_NAMES[int(get_planet_lon(next_am, swe.SUN) / 30.0) % 12]

    return MASA_NAMES[end_rashi]
...
def debug_nakshatra(birth_date: date, birth_lat: float, birth_lon: float,
    ...
    moon_trop = swe.calc_ut(jd, swe.MOON, _CALC_FLAGS)[0][0]
